from numpy import isin, concatenate, intersect1d, array, argsort, empty, argmin
...
def generar_poblacion():
    poblacion = empty((tamano_poblacion, tamano_nodos), 'int')
    for i in range(tamano_poblacion):
        for j, cromosoma in enumerate(sample(range(tamano_nodos), tamano_nodos)):
            poblacion[i, j] = cromosoma
    return poblacion

def costo(cromosoma):
    c = 0
    for i in range(1, tamano_nodos):
        c += matriz_costos[cromosoma[i-1]][cromosoma[i]]
    c += matriz_costos[cromosoma[tamano_nodos-1]][cromosoma[0]]
    return c

def fitness(poblacion):
    costos = empty((tamano_poblacion), 'int')
    for i, cromosoma in enumerate(poblacion):
        costos[i] = costo(cromosoma)
    return costos

def seleccion(poblacion, costos, tamamo_torneo):
    participantes = randint(tamano_poblacion, size=tamamo_torneo)
    return poblacion[participantes[argmin(costos[participantes])]]

def crossover(cromosoma1, cromosoma2):
    c = randint(1, tamano_nodos)
    r11 = cromosoma1[:c]
    r12 = cromosoma1[c:]
    r21 = cromosoma2[:c]
    r22 = cromosoma2[c:]
    c1 = intersect1d(r11, r22)
    c2 = intersect1d(r21, r12)
    r11[isin(r11, c1)] = c2
    r21[isin(r21, c2)] = c1
    cruce1 = concatenate((r11, r22))
    cruce2 = concatenate((r21, r12))
    return cruce1, cruce2

def mutar(cromosoma, tasa_mutacion):
    if random() < tasa_mutacion:
      i = sample(range(1, tamano_nodos), 2)
      i1 = min(i)
      i2 = max(i)
      cromosoma[i1:i2] = cromosoma[i2-1:i1-1:-1]
    return cromosoma

def ga_discreto(proceso=True):
    iteraciones = []
    solucion = None
    p = generar_poblacion()
    c = fitness(p)
    for i in range(generaciones):
        padres = [seleccion(p, c, 2) for j in range(tamano_poblacion)]
        cruzes = []
        ...
        nc = fitness(mutaciones)
        c = concatenate((c, nc))        
        p = concatenate((p, mutaciones))
        indices = argsort(c)
        c = c[indices][:-tamano_poblacion]
        p = p[indices][:-tamano_poblacion]
        indice = argmin(c)
        solucion = p[indice]
        if proceso:
            print(f"{i+1}. permutación: {p[indice]} costo: {c[indice]}")
        iteraciones.append([i, c[indice]])
...
if __name__ == "__main__":
  ...
  ejecuciones = []
  for i in range(10):
    x, y = ga_discreto(proceso=True)
    y = list(zip(*y))
    resultados.append(y[1])
    fig, ax = plt.subplots()
    plt.title(f'Algoritmo Genético para TSP prueba {i+1}')
    plt.xlabel('generaciones')
    plt.ylabel('costo del recorrido')
    ax.text(0.5, -0.4, f"Mejor permutación:\n{x}\nCosto del recorrido: {y[1][len(y[1])-1]}", transform=ax.transAxes, ha='center')
    ax.plot(y[0], y[1])
